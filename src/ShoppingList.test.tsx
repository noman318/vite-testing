...
describe("Shopping List", () => {
  ...
  it("should select ingredients - local spy", async () => {
    ...
    expect(someFunctionSpy).toHaveBeenCalledWith("Apples");
  });

  it("should select ingredients - external spy", async () => {
    const onItemSelectSpy = vi.spyOn(Util, "onItemSelect");
    render(
      <ShoppingList groceries={ingredients} selectItem={Util.onItemSelect} />
    );
    const user = userEvent.setup();
    ...
    expect(onItemSelectSpy).toHaveBeenCalledWith("Apples");
  });

  it("should select ingredients - external spy and Date spy", async () => {
    const onItemSelectSpy = vi.spyOn(Util, "onItemSelectWithTime");
    const dateSpy = vi.spyOn(Date, "now");
    render(
      <ShoppingList
        groceries={ingredients}
        selectItem={Util.onItemSelectWithTime}
      />
    );
    const user = userEvent.setup();
    ...
    const shoppingList = screen.getByRole("list");
    const ingredientItems = within(shoppingList).getAllByRole("listitem");
    const milkIngredient = ingredientItems[1];
    await user.click(milkIngredient);
    expect(onItemSelectSpy).toHaveBeenCalledWith(ingredients[1]);
  ...
  it("should select ingredients - mock - easiest way", async () => {
    ...
    render(
      <ShoppingList groceries={ingredients} selectItem={selectItemMock} />
    );
    const user = userEvent.setup();

    const shoppingList = screen.getByRole("list");
    const ingredientItems = within(shoppingList).getAllByRole("listitem");
    const milkIngredient = ingredientItems[2];
    await user.click(milkIngredient);
    expect(selectItemMock).toHaveBeenCalledWith(ingredients[2]);
